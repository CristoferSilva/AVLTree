...
public class AVLTree<Key extends Comparable <Key>, Value> implements IAVlTree<Key, Value> {
	...
	private AVLNode<Key, Value> getUnbalancedNode(AVLNode<Key, Value> x, Key key, AVLNode<Key, Value> lastUnbalancedNODE){
		if (x == null) {
			return null;
		}
		int compareResult = key.compareTo(x.getKey());
		...
		if (compareResult < 0) {
			if (x.getBalanceFactor() < -1 || x.getBalanceFactor() > 1) {
